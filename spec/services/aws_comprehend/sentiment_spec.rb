...
RSpec.describe AwsComprehend::Sentiment do
  before do
    allow(Aws::Comprehend::Client).to receive(:new).with(region: instance_of(String), access_key_id: instance_of(String), secret_access_key: instance_of(String)).and_return(client)
  end
  ...
  describe '#execute' do
    context 'when the language code exists in the laguage code list' do
      ...
      before do
        allow(client).to receive(:detect_sentiment).with(text: instance_of(String), language_code: instance_of(String)).and_return(detect_sentiment)
      end

      context 'detect sentiment when the text is neutral' do
        include_context 'detect_sentiment', 'NEUTRAL', 0.020090490579605103, 0.002232613507658243, 0.9776280522346497, 4.888398325419985e-05
      ...
      context 'detect sentiment when the text is positive' do
        include_context 'detect_sentiment', 'POSITIVE', 0.9776280522346497, 0.002232613507658243, 0.020090490579605103, 4.888398325419985e-05
      ...
      context 'detect sentiment when the text is negative' do
        include_context 'detect_sentiment', 'NEGATIVE', 0.002232613507658243, 0.9776280522346497, 0.020090490579605103, 4.888398325419985e-05
      ...
      context 'detect sentiment when the text is mixed' do
        include_context 'detect_sentiment', 'MIXED', 0.002232613507658243, 4.888398325419985e-05, 0.020090490579605103, 0.9776280522346497
    ...
    context 'when the language code does not exists in the laguage code list' do
      ...
      before do
        allow(client).to receive(:detect_sentiment).with(text: instance_of(String), language_code: instance_of(String)).and_raise(Aws::Comprehend::Errors::ValidationException.new(nil, exception_message))
      end
